...
covid %>% 
  filter(date == max(date)) %>% 
  group_by(state) %>% 
  summarize(date = max(date), cases = sum(cases)) %>% 
  slice_max(cases, n = 5)
...
covid %>% 
  ...
  select(county, deaths, cases, date, deathratio) %>% 
  filter(cases != 0) %>% 
  filter(date == max(date)) %>% 
  filter(county != "Unknown") %>% 
  slice_max(deathratio, n = 5)
```
...
covid %>% 
  ...
  group_by(state) %>% 
  summarize(deaths = sum(deaths), cases = sum(cases), deathratio = 100*(deaths / cases))%>%
  slice_max(deathratio, n = 10)
